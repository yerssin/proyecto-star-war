	...
	return {
		...
		actions: {
			getPeople: requestURL => {
				fetch(requestURL)
					.then(async res => {
						...
						setStore({
							characters: [...getStore().characters, ...response.results]
						});
						if (response.next) {
			...
			getStarShips: requestURL => {
				fetch(requestURL)
					.then(async res => {
						const response = await res.json();
						setStore({
							starShipsList: [...getStore().starShipsList, ...response.results]
						});
						if (response.next) {
			...
			clickDeleteFavorite: targetIndex => {
				setStore({
					favorites: getStore().favorites.filter(index => index !== targetIndex)
				});
			},
